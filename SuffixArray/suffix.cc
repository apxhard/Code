...
// inlines
inline bool leq(long a1, long a2,   long b1, long b2) { // lexic. order for pairs
...
bool isPermutation(long *SA, long n) {
  ...
  for (long i = 0;  i < n;  i++) seen[i] = 0;
  for (long i = 0;  i < n;  i++) seen[SA[i]-1] = 1;
  for (long i = 0;  i < n;  i++) if (!seen[i]) return 0;
  return 1;
}
...
bool isSorted(long *SA, long *s, long n) {
  for (long i = 0;  i < n-1;  i++) {
    if (!sleq(s+SA[i]-1, s+SA[i+1]-1)) return 0;
  }
  return 1;  
}
  ...
  for (long p=0,  t=n0-n1,  k=0;  k < n;  k++) {
    ...
    { // suffix from SA12 is smaller
      ...
      if (t == n02) { // done --- only SA0 suffixes left
        for (k++;  p < n0;  p++, k++) SA[k] = SA0[p]+1;
      }
    } else { 
      SA[k] = j+1;  p++; 
      if (p == n0)  { // done --- only SA12 suffixes left
        for (k++;  t < n02;  t++, k++) SA[k] = GetI()+1; 
      }
    }  
  } 
...
void ConstructSuffixArray (char *sChar, long n, long *A)
{
  ...
  long b = ASCIIRANGE;
  //Yuck!!  
  for (long i=0;i<n;i++) s[i]=charToLong(sChar[i]);
...
void ConstructHeightArray (char *S, long N, long *A, long *H)
{
  ...
  for (long i=0;i<n;i++) rank[A[i]-1]=i;
...
int lexm(char *s, long m, char *t, long n)
{
  ...
  long answer=-1;
  if (result < 0) { answer=1; goto done;}
  else if (result > 0) {answer= 0;goto done;}
  else {
    if (m <= n) {answer = 1; goto done;}
    else {answer = 0; goto done;}
  }
done:
#ifdef STRSRCH
  cout <<"s: ";
